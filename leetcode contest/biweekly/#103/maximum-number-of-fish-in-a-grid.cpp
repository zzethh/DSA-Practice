class Solution {
public:
//     int helper(vector<vector<int>>& grid,int i,int j,vector<vector<bool>> &v){
//         if(i>=grid.size() or j>=grid[0].size() or i==-1 or j==-1) return 0;
//         if(grid[i][j]==0) return 0;
//         if(v[i][j]) return 0;

//         v[i][j]=1;
//         int down=helper(grid,i+1,j,v);
//         int up=helper(grid,i-1,j,v);
//         int right=helper(grid,i,j+1,v);
//         int left=helper(grid,i,j-1,v);
//         v[i][j]=0;
