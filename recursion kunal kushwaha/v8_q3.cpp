...
int permutation3(string s, string output, int i) {
	if (s[i] == '\0') {
		return 1;
	}
	int cnt = 0;
	...
	for (int j = 0; j <= output.length(); j++) {
		cnt = cnt + permutation3(s, output.substr(0, j) + s[i] + output.substr(j, output.length()), i + 1);
	}
	return cnt;
}
