...
pairxx heightbalanced(node* root) {
	pairxx p;
	if (root == NULL) {
		p.height = 0;
		p.isbalanced = true;
	...
	auto l = heightbalanced(root->left);
	auto r = heightbalanced(root->right);
...
int main() {
	...
	levelOrder(root);
	pairxx p = heightbalanced(root);
	cout << endl;
	cout << p.isbalanced << "x" << p.height;
